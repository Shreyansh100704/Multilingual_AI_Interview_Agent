...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.log("SCRIPT VERSION: FIXED_MERGE_LOGIC_V2");
    const versionBadge = document.createElement('div');
    versionBadge.textContent = "v2.0 (Fix Active)";
    versionBadge.style.position = 'fixed';
    versionBadge.style.bottom = '10px';
    versionBadge.style.right = '10px';
    versionBadge.style.background = '#28a745';
    versionBadge.style.color = 'white';
    versionBadge.style.padding = '5px 10px';
    versionBadge.style.borderRadius = '5px';
    versionBadge.style.fontSize = '12px';
    versionBadge.style.zIndex = '9999';
...
async function startInterview() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/start-interview`, {
            ...
            credentials: 'include',
            body: JSON.stringify({ role, difficulty, model_id: modelId, language, stt_mode: sttMode })
        });
...
function handleSTTToggle(event) {
    ...
    if (event.target.checked) {
        // Google STT enabled
        hinglishToggle.disabled = false;
    } else {
        // Browser STT enabled
        hinglishToggle.disabled = true;
        hinglishToggle.checked = false;
    }
}

function handleLanguageChange(event) {
    const hinglishToggleGroup = document.getElementById('hinglishToggleGroup');

    if (event.target.value === 'hi') {
        hinglishToggleGroup.style.display = 'flex';
    } else {
        hinglishToggleGroup.style.display = 'none';
        document.getElementById('hinglishToggle').checked = false;
    }
}
...
function initializeSpeechRecognition() {
    if ('webkitSpeechRecognition' in window) {
        ...
        recognition.onresult = (event) => {
            ...
            console.log('Current answer box length:', answerBox.value.length);
...
async function transcribeWithGoogle(audioBlob) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('Current answer box length (Google STT):', answerBox.value.length);
            if (answerBox.value.trim()) {
                console.log('Appending to existing text (Google STT)');
                answerBox.value += ' ' + punctuatedText;
            } else {
                console.log('Setting new text (Google STT)');
                answerBox.value = punctuatedText;
            }
